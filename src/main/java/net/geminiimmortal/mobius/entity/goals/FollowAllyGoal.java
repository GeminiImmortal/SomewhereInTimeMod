...
public class FollowAllyGoal extends Goal {
    ...
    private final DiamondGolemEntity diamondsGolem;
    ...
    public FollowAllyGoal(DiamondGolemEntity diamondsGolem, double speedModifier, float followDistance, float minDistance) {
        this.diamondsGolem = diamondsGolem;
    ...
    public boolean canUse() {
        ...
        List<HeartGolemEntity> heartsGolems = diamondsGolem.level.getEntitiesOfClass(HeartGolemEntity.class,
                this.diamondsGolem.getBoundingBox().inflate(followDistance));
    ...
    public boolean canContinueToUse() {
        ...
        double distanceSq = this.diamondsGolem.distanceToSqr(this.heartsGolem);
    ...
    public void stop() {
        ...
        this.diamondsGolem.getNavigation().stop();
    }
    ...
    public void tick() {
        if (--this.delayCounter <= 0) {
            ...
            if (distanceSq > (double)(this.minDistance * this.minDistance)) {
                this.diamondsGolem.getNavigation().moveTo(this.heartsGolem, this.speedModifier);
            } else {
                this.diamondsGolem.getNavigation().stop(); // Stop moving if too close
            }

            // Look at the Hearts Golem while following
            this.diamondsGolem.getLookControl().setLookAt(this.heartsGolem, 30.0F, 30.0F);
        }
    }
}
