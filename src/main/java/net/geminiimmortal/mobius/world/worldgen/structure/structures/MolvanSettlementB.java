...
public class MolvanSettlementB extends Structure<NoFeatureConfig> {

    public MolvanSettlementB() {
    ...
    protected boolean isFeatureChunk(ChunkGenerator chunkGenerator, BiomeProvider biomeSource,
        ...
        if (landHeight > 20) { // Ensure there's enough space above bedrock
            BlockPos cavePosition = centerOfChunk.above(landHeight - 40); // Place roughly 10 blocks below surface
    ...
    public IStartFactory<NoFeatureConfig> getStartFactory() {
        return MolvanSettlementB.Start::new;
    }
        ...
        public void generatePieces(DynamicRegistries dynamicRegistryManager, ChunkGenerator chunkGenerator,
            ...
            JigsawManager.addPieces(dynamicRegistryManager,
                    new VillageConfig(() -> dynamicRegistryManager.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                            .get(new ResourceLocation(MobiusMod.MOD_ID, "molvan/molvan_settlement_b_start_pool")),
                            10), AbstractVillagePiece::new, chunkGenerator, templateManagerIn,
                    blockpos, this.pieces, this.random, false, true);

            this.pieces.forEach(piece -> piece.move(0, -20, 0));
            this.pieces.forEach(piece -> piece.getBoundingBox().y0 -= 1);

            this.calculateBoundingBox();

            LogManager.getLogger().log(Level.DEBUG, "B Settlement at " +
