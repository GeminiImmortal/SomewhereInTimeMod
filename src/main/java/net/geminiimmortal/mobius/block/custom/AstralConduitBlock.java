...
public class AstralConduitBlock extends Block {
    public AstralConduitBlock(Properties properties) {
        super(properties);
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!state.is(newState.getBlock())) { // Check if the block is being replaced by the same type
            ...
            if (tileEntity instanceof AstralConduitTileEntity) {
                // Drop inventory
                ItemStackHandler inventory = ((AstralConduitTileEntity) tileEntity).getItemHandler();
    ...
    private INamedContainerProvider createContainerProvider(World worldIn, BlockPos pos) {
        return new INamedContainerProvider() {
            ...
            public ITextComponent getDisplayName() {
                return new TranslationTextComponent("screen.mobius.astral_conduit");
            }

            @Nullable
            @Override
            public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerEntity) {
                return new AstralConduitContainer(i, worldIn, pos, playerInventory, playerEntity);
            }
        };
    }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntities.ASTRAL_CONDUIT_TILE_ENTITY.get().create();
    }
