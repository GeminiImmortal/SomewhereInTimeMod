...
public class EssenceChannelerBlock extends Block {
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos,
        ...
        if(!worldIn.isClientSide()) {
            ...
            if(!player.isCrouching()) {
                if(tileEntity instanceof EssenceChannelerTileEntity) {
    ...
    public void onRemove(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!state.is(newState.getBlock())) { // Check if the block is being replaced by the same type
            ...
            if (tileEntity instanceof EssenceChannelerTileEntity) {
                // Drop inventory
                ItemStackHandler inventory = ((EssenceChannelerTileEntity) tileEntity).getItemHandler();
    ...
    private INamedContainerProvider createContainerProvider(World worldIn, BlockPos pos) {
        return new INamedContainerProvider() {
            ...
            public ITextComponent getDisplayName() {
                return new TranslationTextComponent("screen.mobius.essence_channeler");
            }

            @Nullable
            @Override
            public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerEntity) {
                return new EssenceChannelerContainer(i, worldIn, pos, playerInventory, playerEntity);
            }
        };
    }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntities.ESSENCE_CHANNELER_TILE_ENTITY.get().create();
    }
