...
public class TitleUtils {
    public static void sendTitle(ServerPlayerEntity player, String title, String subtitle, int fadeIn, int stay, int fadeOut, TextFormatting titleColor) {
        // Set timing
        player.connection.send(new STitlePacket(fadeIn, stay, fadeOut));

        // Send title
        if (title != null && !title.isEmpty()) {
            player.connection.send(new STitlePacket(Type.TITLE, new StringTextComponent(titleColor + title)));
        }
